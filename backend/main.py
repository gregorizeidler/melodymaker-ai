...
app = modal.App("melodymaker-ai")

image = (
    modal.Image.debian_slim()
    .apt_install("git")
    .uv_pip_install("transformers", "diffusers", "pydantic", "torch", "fastapi[standard]", "boto3")
...
melodymaker_secrets = modal.Secret.from_name("melodymaker-secret")

# @app.function(image=image, secrets=[modal.Secret.from_name("melodymaker-secret")])
# def func_test():
...
@app.cls(image=image, secrets=[melodymaker_secrets], gpu="L40S", volumes={"/models": modal_volume, "/.cache/huggingface": hf_volume}, scaledown_window=15)
class MusicGenServer:
    ...
    def generate_with_description(self, request: GenerateFromDescriptionRequest) -> GenerateMusicResponseS3:
...
def main():
    ...
    print("Audio S3 Key:", result.s3_key)
